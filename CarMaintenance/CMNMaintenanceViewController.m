//
//  CMNMaintenanceViewController.m
//  CarMaintenance
//
...
@interface CMNMaintenanceViewController ()<UITableViewDelegate,UITableViewDataSource>
@property (nonatomic, strong) UITableView *cmn_car_tableView;
@property (nonatomic, strong) NSArray *cmn_car_dataSource;
@end

@implementation CMNMaintenanceViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    [self.navigationItem setTitle:@"保养记录"];
    ...
    UIButton *cmn_car_maintenance_nav_right_button = [UIButton buttonWithType:UIButtonTypeSystem];
    [cmn_car_maintenance_nav_right_button setTitle:@"添加" forState:UIControlStateNormal];
    cmn_car_maintenance_nav_right_button.titleLabel.font = [UIFont boldSystemFontOfSize:18];
    [cmn_car_maintenance_nav_right_button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [cmn_car_maintenance_nav_right_button addTarget:self action:@selector(cmn_car_maintenance_nav_right_buttonClick:) forControlEvents:UIControlEventTouchUpInside];
    
    UIBarButtonItem *rightItem = [[UIBarButtonItem alloc] initWithCustomView:cmn_car_maintenance_nav_right_button];
    self.navigationItem.rightBarButtonItem = rightItem;
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reloadDdata) name:KCarMaintenanceRecordAddSuccessNotificationName object:nil];
}

- (void)reloadDdata {
    self.cmn_car_dataSource = [self get_cmn_car_dataSource];
    [self.cmn_car_tableView reloadData];
}

- (NSArray *)get_cmn_car_dataSource {
    NSArray *dataList = [NSArray arrayWithArray:[NSKeyedUnarchiver unarchiveObjectWithFile:KCarMaintenanceRecordPath]];
    return dataList;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    CMNMaintenanceRecordCell *cell = [CMNMaintenanceRecordCell cellWithTableView:tableView];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row < self.cmn_car_dataSource.count) {
        CMNAddCarMaintenanceViewController *vc = [CMNAddCarMaintenanceViewController new];
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    if (section == 0 && self.cmn_car_dataSource.count == 0) {
        ...
        titleLabel.text = @"还没有爱车保养记录哦，赶快去添加吧～";
...
- (void)cmn_car_maintenance_nav_right_buttonClick:(UIButton *)button {
    CMNAddCarMaintenanceViewController *vc = [CMNAddCarMaintenanceViewController new];
    vc.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:vc animated:YES];
}
